...
const EmployeesForm = (props) => {
    ...
    const validate = (fieldValues = values) => {
        ...
        if ('phone' in fieldValues)
            temp.phone = fieldValues.phone && (/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im).test(fieldValues.phone)// eslint-disable-line
                ? ""
        ...
        if ('confirmPassword' in fieldValues)
            temp.confirmPassword = values.password === fieldValues.confirmPassword
                ? ""
                : "Паролите не съвпадат"
        if ('role' in fieldValues)
            temp.role = fieldValues.role.length !== 0 ? "" : "Това поле е задължително"
        if ('workLocation' in fieldValues)
            temp.workLocation = fieldValues.workLocation.length !== 0 ? "" : "Това поле е задължително"
    ...
    const registerHandler = async (e) => {
        ...
        if (validate()) {
            ...
            try {
                ...
                if (values.id === 0) {
                    addOne('/api/users/register', userObject)
