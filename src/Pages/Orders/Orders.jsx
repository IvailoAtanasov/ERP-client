...
import { InputAdornment, makeStyles, Paper, TableSortLabel, Toolbar } from '@material-ui/core';
...
import { registerOne, updateOne, getAll, deleteOne } from '../../Services/EmployeeService/EmployeeService';
...
const Orders = () => {
    ...
    const ordersAfterPagingandSorting = () => {
        return stableSort(filterFn.fn(orders), getComparator(order, orderBy)).slice(page * rowsPerPage, (page + 1) * rowsPerPage)
    }
        ...
        setFilterFn({
            fn: items => {
                if (target.value === '')
                    ...
                    return items.filter(x => getStatusNameById(x.status).toLowerCase().includes(target.value.toLowerCase())
                                ...
                                <TableRow key={item._id}>
                                    ...
                                    <TableCell>
                                        ...
                                        <Controls.ActionButton color="secondary"
                                            onClick={() => { deleteOrder(`/api/orders/order/${item._id}`) }} >
